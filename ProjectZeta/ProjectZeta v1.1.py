...
class Clipboard(object):
    #УБРАЛ ОТСЮДА СЧЕТЧИК
    def __init__(self, rdate=None, record=None, teg=None):
        self.rdate = rdate
        self.record = record
        self.teg = teg
        
    def __str__(self):
        return ("{} {:<30} {}").format(self.rdate, self.record, self.teg)
...
class ClipboardAPP(object):
    ...
    #ОТСЮДА ТОЖЕ УБРАЛ СЧЕТЧИК, ДОБАВИЛ ВВОД ТЕГОВ
    def getdata(self):
    ...
    def add_cb(self):
        ...
        self.records[record] = Clipboard(rdate, record, tag)
        print('Record stored!')

    #СЮДА ДОБАВИЛ ДИНАМИЧЕСКИЙ СЧЕТЧИК
    def view_cb(self):
        if self.records:
            print("{:<5} {:<10} {:<30} {}".format("№", "date", "record", "tegs"))
            print('---------------------------------------------------------------------------------------')
            counter_rec = 1
            for record in self.records.values(): 
                print(counter_rec,'.', record)
                print('---------------------------------------------------------------------------------------')
                counter_rec+=1
        else:
...
class Application(object):
    ...
    def viewall(self):
        if self.persons:
            print("{:<20} {:<20} {:<15} {:<15} {}".format('Name', 'address', 'Phone', 'Email', 'Birthday'))
            print('---------------------------------------------------------------------------------------')
    ...
    def getdetails(self):
        ...
        def CorrectEmail(email):
            match = re.search(r"[\w\.-]+@[\w\.-]+(\.[\w]{3,5})+", email)
            if match:
                return True
            return (False, 'Неверный электронный адрес')
